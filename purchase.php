<?php
	...
	if ( isset($_POST['sdate']) && isset($_POST['iid']) && isset($_POST['stype']) && isset($_POST['sinvoiceno']) && isset($_POST['suprice']) && isset($_POST['sqty']) ){
		...
		$sql_q="INSERT INTO stock_purchase VALUES ('', '$sdate', '$iid', '$stype', '$sinvoiceno', '$suprice', '$sqty', '$scomments');";
-
		$result = mysql_query($sql_q);

		if ( $result )
			echo '<script type="text/javascript">window.alert("Purchase record Added Successfully")</script>';
		else
			echo '<script type="text/javascript">window.alert("ERROR!! Failed to Add Purchase record")</script>';
	}
?>
<html>
<head>
	...
	<div id="popupForm">
		<table width=100% cellspacing="5">
		 <form name="addform" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">		
			<tr>
				<td colspan="2" align="center" valign="center"><h2>Add Purchase</h2></td>
